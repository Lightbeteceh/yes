...
class YesCoinBot {
        ...
        try {
            ...
            if (response.code === 0) {
                if (response.data.collectStatus) {
                    await this.log(`Opened the box and received ${response.data.collectAmount} Coins`, 'success');
    ...
    async getTaskList(token, proxy) {
        ...
        try {
            ...
            if (response.code === 0) {
                ...
                await this.log(`Could not retrieve the task list: ${response.message}`, 'error');
                return null;
            }
        } catch (error) {
    ...
    async finishTask(token, taskId, proxy) {
        ...
        try {
            ...
            if (response.code === 0) {
                await this.log(`Successfully completed task ${taskId} | Reward: ${response.data.bonusAmount}`, 'success');
                return true;
            } else {
                await this.log(`Failed to complete task ${taskId}: ${response.message}`, 'error');
                return false;
            }
        } catch (error) {
            ...
            if (accountBuildInfo.code === 0) {
                ...
                if (swipeBotLevel < 1) {
                    ...
                    if (upgradeResponse.code === 0) {
                        await this.log('SwipeBot purchase successful', 'success');
                    } else {
                ...
                if (swipeBotLevel >= 1 && !openSwipeBot) {
                    ...
                    if (toggleResponse.code === 0) {
                        await this.log('SwipeBot activated successfully', 'success');
                    } else {
                        await this.log('SwipeBot activation failed', 'error');
                    }
                }
                ...
                if (swipeBotLevel >= 1 && openSwipeBot) {
                    ...
                    if (offlineBonusInfo.code === 0 && offlineBonusInfo.data.length > 0) {
                        ...
                        if (claimResponse.code === 0) {
                            await this.log(`Successfully claimed offline bonus, received ${claimResponse.data.collectAmount} coins`, 'success');
                        } else {
                            await this.log('Offline bonus claim failed', 'error');
                        }
                    }
                }
            } else {
                await this.log('Failed to retrieve SwipeBot info', 'error');
            }
        } catch (error) {
            await this.log(`SwipeBot handling error: ${error.message}`, 'error');
        }
    }
    ...
    async main() {
        ...
        try {
            ...
            await this.log(`Failed to retrieve token: ${error.message}`, 'error');
            return;
        }
    ...
    async performTasks() {
        ...
        if (squadInfo && squadInfo.data.isJoinSquad) {
            ...
            await this.log('Squad: You are not part of a Squad, join Dân Cày Airdrop.', 'warning');
            await this.randomDelay();
        ...
        if (gameInfo === null) {
            await this.log('Failed to retrieve game data!', 'error');
        } else {
        ...
        if (collectInfo === null) {
            ...
            await this.log(`Energy left: ${coinPoolLeftCount}`, 'info');
...
if (isMainThread) {
    ...
    async function processCycle() {
        console.log('If you use it, don’t be afraid; if you are afraid, don’t use it...'.magenta);
        let accountQueue = [...accounts];

        function startWorker() {
            if (accountQueue.length === 0) {
                if (activeWorkers === 0) {
                    console.log('All accounts completed, taking a short break.'.green);
            ...
            worker.on('exit', (code) => {
                if (code !== 0) {
                    console.error(`Thread stopped with code ${code}`.red);
                }
                activeWorkers--;
                startWorker();
            });
